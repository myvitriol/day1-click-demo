...
VERSION = "v07"
SLUG = "dial-3step"
NOTE = ("v06 의 dial 을 연속에서 3단(느슨/보통/엄격) 고정으로 — 시험이 12번뿐이라 "
        "연속 값은 없는 정밀도를 약속하고, 현장에서 값을 더듬는 모습도 막는다")
...
HOT, OK = "#ff7a3d", "#22c55e"
GRAY_DOT = (201, 198, 200, 115)      # 그 밖의 소리
BLK_DOT = (51, 49, 50, 130)          # 기존 체결음
PINK_SOFT, BLK_SOFT = "#fff0f2", "#f2f1f2"
PINK_DEEP = "#d3213f"                # 헤더 전용

# 본문 accent — 브랜드 색과 분리한다 (검출 · 이 이벤트)
ACC, ACC_SOFT, ACC_DEEP = "#00b39c", "#e8faf7", "#00907e"
...
txt(d, 600, 23, "정지 — 방금 잡은 소리 보는 중", f(KRB, 11), PINK_DEEP)
...
for i in range(n):
    ...
    d.line([xx, wcy * S - v * S, xx, wcy * S + v * S],
           fill=ACC if ax * S <= xx <= bx * S else "#9c999b", width=1)

# "배경보다 이만큼 크다" = SNR 을 숫자 대신 높이로
arx = bx + 26
pk = wcy - .92 * wamp
d.line(px(arx, pk, arx, wcy - bh), fill=ACC_DEEP, width=int(1.6 * S))
for yy, dy in ((pk, 6), (wcy - bh, -6)):
    d.polygon([px(arx - 4, yy + dy), px(arx + 4, yy + dy), px(arx, yy)], fill=ACC_DEEP)
d.line(px(ax, pk, arx + 8, pk), fill=(0, 179, 156, 115), width=S)
txt(d, arx + 12, (pk + wcy - bh) / 2 - 16, "배경보다", f(KR, 10.5), ACC_DEEP)
txt(d, arx + 12, (pk + wcy - bh) / 2 - 2, "18 dB", f(MNB, 17), ACC_DEEP)
txt(d, arx + 12, (pk + wcy - bh) / 2 + 18, "더 큼", f(KR, 10.5), ACC_DEEP)

caption(x0 + 22, y1 - 76,
        "회색 띠가 이 방의 평소 소음. 청록으로 크게 솟은 것이 방금 체결한 소리다.")
caption(x0 + 22, y1 - 54,
...
gx0, gy0 = x0 + 22, y0 + 50
gx1, gy1 = x1 - 22, y1 - 222
gw2, gh2 = gx1 - gx0, gy1 - gy0
AR = gw2 / gh2
mp = lambda u, v: ((gx0 + u * gw2) * S, (gy0 + v * gh2) * S)

# 판정 원 = threshold. dial 을 돌리면 이 반지름이 움직인다
CU, CV = .30, .40
R_LOOSE, R_CUR, R_TIGHT = .250, .175, .115
ccx, ccy = mp(CU, CV)
at = lambda rf, deg: (ccx + math.cos(math.radians(deg)) * rf * gw2 * S,
                      ccy + math.sin(math.radians(deg)) * rf * gw2 * S)

CL = [(CU, CV, .135, BLK_DOT, 85),
      (.52, .80, .090, GRAY_DOT, 40),
      (.76, .46, .235, GRAY_DOT, 160)]
for cu, cv, r, col, cnt in CL:
    for _ in range(cnt):
...
# 기준을 풀면 딸려 들어올 소음 / 조이면 놓칠 체결음
for deg in (-142, -38, 62):
    gx, gy = at((R_CUR + R_LOOSE) / 2, deg)
    d.ellipse([gx - 3.4 * S, gy - 3.4 * S, gx + 3.4 * S, gy + 3.4 * S], fill=(160, 157, 159, 190))
for deg in (-108, -14, 128):
    bx2, by2 = at((R_TIGHT + R_CUR) / 2 + .012, deg)
    d.ellipse([bx2 - 3.4 * S, by2 - 3.4 * S, bx2 + 3.4 * S, by2 + 3.4 * S], fill=(51, 49, 50, 200))

for rf, deg, anc, lab in ((R_LOOSE, -78, "la", "느슨하게"), (R_TIGHT, 214, "ra", "엄격하게")):
    rad = rf * gw2 * S
    for a in range(0, 360, 11):
        a0, a1 = math.radians(a), math.radians(a + 6)
        d.line([ccx + math.cos(a0) * rad, ccy + math.sin(a0) * rad,
                ccx + math.cos(a1) * rad, ccy + math.sin(a1) * rad],
               fill=(120, 118, 119, 90), width=int(1.4 * S))
    lx2, ly2 = at(rf, deg)
    dx2 = 5 if anc == "la" else -5
    txt(d, lx2 / S + dx2, ly2 / S - 6, lab, f(KR, 9.5), TX2, anchor=anc)

rad = R_CUR * gw2 * S
d.ellipse([ccx - rad, ccy - rad, ccx + rad, ccy + rad], outline=ACC, width=int(2.2 * S))
lx2, ly2 = at(R_CUR, -20)
txt(d, lx2 / S + 6, ly2 / S - 7, "지금 기준", f(KRB, 10), ACC_DEEP)

txt(d, *[v / S for v in mp(.30, .05)], "기존 체결음", f(KRB, 11), BLK, anchor="mm")
txt(d, *[v / S for v in mp(.80, .13)], "그 밖의 소리", f(KRB, 11), TX3, anchor="mm")

sx, sy = mp(.335, .465)
for rr, al in ((26, 60), (17, 105)):
    d.ellipse([sx - rr * S, sy - rr * S, sx + rr * S, sy + rr * S],
              outline=(0, 179, 156, al), width=int(2 * S))
star = []
for i in range(10):
    a = -math.pi / 2 + i * math.pi / 5
    rr = (4.2 if i % 2 else 10.5) * S
    star.append((sx + math.cos(a) * rr, sy + math.sin(a) * rr))
d.polygon(star, fill=ACC)

cxx, cyy = sx / S + 30, sy / S - 42
d.line(px(sx / S + 13, sy / S - 10, cxx - 4, cyy + 14), fill=(0, 179, 156, 165), width=int(1.4 * S))
l1, l2 = "★ 방금 그 소리", "기존 체결음 무리 안에 들어왔다"
bw2 = max(tw(d, l1, f(KRB, 11)), tw(d, l2, f(KR, 10.5))) + 20
d.rounded_rectangle(px(cxx, cyy, cxx + bw2, cyy + 44), radius=int(8 * S), fill=ACC_SOFT)
txt(d, cxx + 10, cyy + 6, l1, f(KRB, 11), ACC_DEEP)
txt(d, cxx + 10, cyy + 24, l2, f(KR, 10.5), "#3f7d73")

caption(x0 + 22, y1 - 206,
        "원 안에 들어오면 체결음으로 판정한다. 기준을 조이면 원이 작아진다.")

# ── 판정 기준 dial — operating point 를 손으로 옮겨 본다 ─────────
d.line(px(x0 + 22, y1 - 182, x1 - 22, y1 - 182), fill=BLK_SOFT, width=S)
txt(d, x0 + 22, y1 - 172, "판정 기준", f(KRB, 12), TX)
txt(d, x0 + 22 + 62, y1 - 169, "3단으로만 바뀝니다", f(KR, 9.5), TX3)

# 3단 고정 — 연속이면 없는 정밀도를 약속하게 된다 (시험이 12번뿐이라 값이 계단이다)
STEPS = ["느슨", "보통", "엄격"]
CUR = 1
ang = lambda i: math.radians(150 + i / (len(STEPS) - 1) * 240)

dcx, dcy, R = x0 + 92, y1 - 96, 46
bb = px(dcx - R, dcy - R, dcx + R, dcy + R)
d.arc(bb, 150, 390, fill="#e9e7e8", width=int(9 * S))
d.arc(bb, 150, 150 + CUR / (len(STEPS) - 1) * 240, fill=ACC, width=int(9 * S))
for i in range(len(STEPS)):                       # 멈추는 자리를 눈에 보이게
    a = ang(i)
    px_, py_ = dcx + R * math.cos(a), dcy + R * math.sin(a)
    d.ellipse(px(px_ - 3, py_ - 3, px_ + 3, py_ + 3),
              fill="#ffffff" if i <= CUR else "#c9c7c8")
ka = ang(CUR)
kx, ky = dcx + R * math.cos(ka), dcy + R * math.sin(ka)
d.ellipse(px(kx - 10, ky - 10, kx + 10, ky + 10), fill=CARD, outline=ACC, width=int(3 * S))
txt(d, dcx, dcy + 2, STEPS[CUR], f(KRB, 17), TX, anchor="mm")

fn, fnb = f(KR, 9.5), f(KRB, 9.5)
ws = [tw(d, s, fnb if i == CUR else fn) for i, s in enumerate(STEPS)]
PAD, GAP = 7, 6
xx = dcx - (sum(w + (2 * PAD if i == CUR else 0) for i, w in enumerate(ws)) + GAP * 2) / 2
yy = dcy + R + 8
for i, s in enumerate(STEPS):
    if i == CUR:
        d.rounded_rectangle(px(xx, yy - 3, xx + ws[i] + 2 * PAD, yy + 15), radius=int(9 * S), fill=ACC)
        txt(d, xx + PAD, yy, s, fnb, "#ffffff")
        xx += ws[i] + 2 * PAD + GAP
    else:
        txt(d, xx, yy, s, fn, TX3)
        xx += ws[i] + GAP

# 이번 세션 시험을 지금 기준으로 다시 채점한 결과
bxs, bxe = x0 + 176, x1 - 22
for i, (nm, val, frac, col) in enumerate([
        ("잡아낸 비율", "8번 중 7번", .875, ACC),
        ("잘못 잡은 것", "4번 중 0번", .0, BLK)]):
    yy = y1 - 152 + i * 52
    txt(d, bxs, yy, nm, f(KR, 11), TX2)
    txt(d, bxe, yy, val, f(KRB, 11.5), col, anchor="ra")
    d.rounded_rectangle(px(bxs, yy + 19, bxe, yy + 30), radius=int(5 * S), fill="#eeecec")
    if frac > 0:
        d.rounded_rectangle(px(bxs, yy + 19, bxs + (bxe - bxs) * frac, yy + 30),
                            radius=int(5 * S), fill=col)
    else:
        d.rounded_rectangle(px(bxs, yy + 19, bxs + 7, yy + 30), radius=int(5 * S), fill="#d8d5d6")

txt(d, bxs, y1 - 42, "기준을 조이면  잘못 잡는 것 ↓  ·  놓치는 것 ↑", f(KR, 10), TX2)
txt(d, bxs, y1 - 26, "이 방에서 해본 12번을 지금 기준으로 다시 채점한 값", f(KR, 9), TX3)
