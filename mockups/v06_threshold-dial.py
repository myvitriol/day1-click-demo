...
VERSION = "v06"
SLUG = "threshold-dial"
NOTE = ("v05 + 판정 기준 dial — 지도의 판정 원 반지름을 손으로 옮기며 "
        "recall/precision trade-off(operating point)를 그 자리에서 보여준다")
...
txt(d, x0 + 22 + 62, y1 - 169, "돌려서 바꿔 보세요", f(KR, 9.5), TX3)

DIAL = .55                                   # 0=느슨 … 1=엄격
dcx, dcy, R = x0 + 92, y1 - 96, 46
bb = px(dcx - R, dcy - R, dcx + R, dcy + R)
d.arc(bb, 150, 390, fill="#e9e7e8", width=int(9 * S))
d.arc(bb, 150, 150 + DIAL * 240, fill=ACC, width=int(9 * S))
ka = math.radians(150 + DIAL * 240)
kx, ky = dcx + R * math.cos(ka), dcy + R * math.sin(ka)
d.ellipse(px(kx - 10, ky - 10, kx + 10, ky + 10), fill=CARD, outline=ACC, width=int(3 * S))
txt(d, dcx, dcy - 4, "보통", f(KRB, 15), TX, anchor="mm")
txt(d, dcx, dcy + 15, "0.55", f(MN, 10), TX3, anchor="mm")
txt(d, dcx, dcy + R + 6, "← 느슨          엄격 →", f(KR, 9.5), TX3, anchor="ma")
