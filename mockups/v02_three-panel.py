...
v02: three panels only. Waveform + spectrogram (shared time window) on the left,
...
VERSION = "v02"
SLUG = "three-panel"
NOTE = ("세 뷰(파형·스펙트로그램·지도)만으로 완결 · 진단/판정/성적표를 각 뷰 안으로 흡수 · "
        "패널마다 평이한 한 줄 캡션 — 처음 보는 사람 기준")
...
def caption(x, y, s):
    ...
    txt(d, x, y, s, f(KR, 12), "#47566b")


# ═══ 헤더 (얇게) ════════════════════════════════════════════════
d.rectangle(px(0, 0, 1280, 54), fill=CARD)
d.line(px(0, 54, 1280, 54), fill=LINE, width=S)
d.ellipse(px(24, 23, 33, 32), fill=OK)
txt(d, 44, 14, "Scarlett 2i2 USB", f(KRB, 13), TX)
txt(d, 44, 31, "외장 마이크 연결됨 · 48 kHz", f(KR, 9.5), TX2)

d.rounded_rectangle(px(300, 12, 496, 42), radius=int(15 * S), fill="#fff3ea")
d.rectangle(px(318, 20, 322, 34), fill=HOT)
d.rectangle(px(326, 20, 330, 34), fill=HOT)
txt(d, 340, 19, "정지 — 방금 잡은 소리 보는 중", f(KRB, 11), "#d9612a")

txt(d, 1262, 12, "오늘 잡은 클릭", f(KR, 10), TX3, anchor="ra")
txt(d, 1262, 24, "47", f(MNB, 23), HOT, anchor="ra")
txt(d, 1053, 20, "세션", f(KR, 10), TX3)
txt(d, 1140, 19, "00:12:34", f(MN, 12), TX2, anchor="ra")
...
bh = .135 * wamp
d.rectangle(px(wx0, wcy - bh, wx1, wcy + bh), fill=(148, 163, 184, 42))
for yy in (wcy - bh, wcy + bh):
    for xx in range(int(wx0), int(wx1), 9):
        d.line(px(xx, yy, xx + 5, yy), fill=(100, 116, 139, 130), width=S)
txt(d, x0 + 76, wcy - bh - 7, "배경 소음", f(KRB, 10), "#64748b", anchor="rd")
...
for i in range(n):
    ...
    d.line([xx, wcy * S - v * S, xx, wcy * S + v * S],
           fill=HOT if ax * S <= xx <= bx * S else "#2fc9b6", width=1)

# "배경보다 이만큼 크다" = SNR 을 숫자 대신 높이로
arx = bx + 26
pk = wcy - .92 * wamp
d.line(px(arx, pk, arx, wcy - bh), fill="#d9612a", width=int(1.6 * S))
for yy, dy in ((pk, 6), (wcy - bh, -6)):
    d.polygon([px(arx - 4, yy + dy), px(arx + 4, yy + dy), px(arx, yy)], fill="#d9612a")
d.line(px(ax, pk, arx + 8, pk), fill=(217, 97, 42, 90), width=S)
txt(d, arx + 12, (pk + wcy - bh) / 2 - 16, "배경보다", f(KR, 10.5), "#d9612a")
txt(d, arx + 12, (pk + wcy - bh) / 2 - 2, "18 dB", f(MNB, 17), "#d9612a")
txt(d, arx + 12, (pk + wcy - bh) / 2 + 18, "더 큼", f(KR, 10.5), "#d9612a")

caption(x0 + 22, y1 - 76,
        "회색 띠가 이 방의 평소 소음. 주황으로 크게 솟은 것이 방금 체결한 소리다.")
caption(x0 + 22, y1 - 54,
        "띠 위로 높이 솟을수록 잡기 쉽다 — 이 소리는 배경보다 18 dB 크다.")
d.line(px(x0 + 22, y1 - 28, x1 - 22, y1 - 28), fill="#f1f5f9", width=S)
...
ay2 = sy0 + (sy1 - sy0) * .44
for rw, rh, al in ((58, 84, 32), (38, 56, 44), (20, 30, 58)):
    d.ellipse(px(ax2 - rw * .3, ay2 - rh / 2, ax2 + rw * .9, ay2 + rh / 2), fill=(255, 122, 61, al))
d.ellipse(px(ax2 - 58 * .3, ay2 - 42, ax2 + 58 * .9, ay2 + 42),
          outline=(217, 97, 42, 165), width=int(1.4 * S))
lb = "모델이 주목한 부분"
lw = tw(d, lb, f(KRB, 10)) + 16
d.rounded_rectangle(px(ax2 + 58, ay2 - 66, ax2 + 58 + lw, ay2 - 44), radius=int(6 * S), fill="#fff3ea")
txt(d, ax2 + 66, ay2 - 61, lb, f(KRB, 10), "#d9612a")
d.line(px(ax2 + 30, ay2 - 40, ax2 + 58, ay2 - 48), fill=(217, 97, 42, 140), width=int(1.2 * S))

caption(x0 + 22, y1 - 76,
        "같은 소리를 색으로 편 그림. 진할수록 그 높이의 소리가 세다.")
caption(x0 + 22, y1 - 54,
        "체결음은 낮은 음부터 높은 음까지 한꺼번에 퍼져서 세로줄 하나로 보인다.")
d.line(px(x0 + 22, y1 - 28, x1 - 22, y1 - 28), fill="#f1f5f9", width=S)
txt(d, x0 + 22, y1 - 22, "주황 영역 표시는 목업 — 모델 미연결", f(KR, 9.5), TX3)
...
CL = [(.30, .38, .135, (0, 191, 166, 130), 85, True),
      (.52, .78, .095, (124, 140, 255, 118), 45, False),
      (.74, .46, .245, (150, 165, 185, 95), 165, False)]
        ...
        for m, al in ((1.0, 80), (1.75, 32)):
            ...
            for a in range(0, 360, 10):
                ...
                d.line([cx + math.cos(a0) * rad, cy + math.sin(a0) * rad,
                       ...
                       fill=(0, 191, 166, al), width=int(1.3 * S))
    for _ in range(cnt):
...
txt(d, *[v / S for v in mp(.30, .16)], "기존 체결음", f(KRB, 11), "#079e8b", anchor="mm")
txt(d, *[v / S for v in mp(.80, .17)], "그 밖의 소리", f(KRB, 11), "#8fa0b8", anchor="mm")
...
d.line(px(sx / S + 13, sy / S - 10, cxx - 4, cyy + 14), fill=(217, 97, 42, 150), width=int(1.4 * S))
l1, l2 = "★ 방금 그 소리", "기존 체결음 무리 안에 들어왔다"
bw2 = max(tw(d, l1, f(KRB, 11)), tw(d, l2, f(KR, 10.5))) + 20
d.rounded_rectangle(px(cxx, cyy, cxx + bw2, cyy + 44), radius=int(8 * S), fill="#fff3ea")
txt(d, cxx + 10, cyy + 6, l1, f(KRB, 11), "#d9612a")
...
d.line(px(x0 + 22, y1 - 122, x1 - 22, y1 - 122), fill="#f1f5f9", width=S)
txt(d, x0 + 22, y1 - 110, "이번 자리에서 해본 시험", f(KRB, 11.5), TX)
for i, (t1, big, t2, col, bgc) in enumerate([
        ("체결해 봤을 때", "8번 중 7번", "잡았다", ACC, "#effcf9"),
        ("헷갈릴 소리를 냈을 때", "4번 중 0번", "잘못 잡았다", "#3b82f6", "#eef4ff")]):
    bxx = x0 + 22 + i * 226
