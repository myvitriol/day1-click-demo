...
노트북 한 대에서 전부 돈다 — Python 이 마이크(96 kHz)와 모델(DAL hdtransys v4_e20)을
독점하고, 브라우저는 화면만 그린다. 인증서·VPN·사내 서버가 필요 없다.
...
```
~/work/
├── DAL/                # 엔진 (private repo, pinned commit)
...
실측: golden(클릭 2개) → **2건 정확**, sim_dense 120초(24개) → **24건 정확·FA 0**,
