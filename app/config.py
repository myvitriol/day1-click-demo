...
SNAP_PRE_S, SNAP_POST_S = 0.5, 0.5     # freeze snapshot 구간(이벤트 t 기준)
#   좌우 대칭 = 잡힌 순간이 화면 **가운데**. 데모에서 그게 훨씬 잘 읽힌다(사용자 결정).
#   c1 은 "딱" 뒤 0.36s 에 두 번째 성분이 따라오는 이중음인데, POST 0.5s 면 그것도
#   구간 안(가운데 기준 +72%)에 들어온다. 웹은 이 비율을 hello 의 snap_pre_frac 으로
#   받아 쓰므로 여기만 바꾸면 세로선·라벨이 함께 움직인다.
