...
#
#   2차 상향(같은 날). 0.93 에서도 **느슨 단계에서조차 너무 많이 잡힌다**는 현장 피드백.
#   위 스윕에서 0.995 까지 손실이 0이었으니 3단을 한 칸씩 올린다: 0.95 / 0.97 / 0.99.
#   기본이 0.97 이면 clean 최저 0.959 보다 높아 그 window 는 놓치지만, 같은 클릭의
#   나머지 4개 window 가 1.000 이라 이벤트 자체는 유지된다(아래 재검증에서 48/48).
#   그래도 FA 가 남으면 문턱 문제가 아니다 — FA 의 점수를 로그로 봐야 한다.
#
...
TAU_ON, TAU_OFF, MIN_ON = 0.97, 0.50, 2
#   TAU_OFF 는 낮게 둔다. 올리면 잡음이 더 쉽게 release 되어 같은 잡음이 반복 카운트된다.
LEVEL_TAU_ON = {-1: 0.95, 0: 0.97, 1: 0.99}    # 판정 기준 3단 → tau_on
